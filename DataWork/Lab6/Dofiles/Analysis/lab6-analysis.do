...
* Task 2: Balance table
	...
	* Balance tables
		...
		* for household head gender, household size and all the income variables
		iebaltab  hh_head_gender hh_hhsize hh_inc_* , grpvar(hh_treatment) ///
		...
		* (treatment and control combined) is also added
		iebaltab  hh_head_gender hh_hhsize hh_inc_* , grpvar(hh_treatment) total 	///
			save("$Lab6_outRaw/balance_2") replace rowvarlabel
		...
		* the bottom of the table.
		iebaltab  hh_head_gender hh_hhsize hh_inc_* , grpvar(hh_treatment) total 	///
			balmiss(zero) ftest														///
			save("$Lab6_outRaw/balance_3") replace rowvarlabel						///
